...
def get_page_id(user_name):
    """获取指定用户的 page_id"""
    if user_name not in page_ids:
        # 如果不存在，生成并返回新的 page_id
        page_ids[user_name] = generate_page_id(user_name)
    return page_ids[user_name]

def set_page_id(user_name, new_page_id):
    """设置指定用户的 page_id"""
    page_ids[user_name] = new_page_id

def generate_page_id(user_name):
    """为每个用户生成一个独特的 page_id"""
    import uuid
    # 这里使用 uuid 生成一个唯一的 page_id，可以根据需要调整生成逻辑
    return str(uuid.uuid4())


def LightRAG_predict(current_model, *args):
    if current_model:
        iter = current_model.LightRAG_predict(*args)
        for i in iter:
            yield i
...
# # 在 LightRAG_predict 和 predict 方法中使用异步生成器
# async def LightRAG_predict(current_model, *args):
#     if current_model:
#         iter = current_model.LightRAG_predict(*args)
#         for i in iter:
#             yield i

# async def predict(current_model, *args):
#     if current_model:
#         iter = current_model.predict(*args)
#         for i in iter:
#             yield i
...
# # 提交任务
# def submit_task_to_queue(task_function, *args):
#     task_queue.append((task_function, args))

# # 执行队列中的任务
# async def execute_task_from_queue():
#     while task_queue:
#         task_function, args = task_queue.pop(0)
