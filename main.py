...
with gr.Blocks(theme=small_and_beautiful_theme) as demo:
                    ...
                    with gr.Tab(label=i18n("对话")):
                        ...
                        with gr.Accordion(label=i18n("知识库"), open=True, elem_id="gr-kb-accordion", visible=True):
                            use_websearch_checkbox = gr.Checkbox(label=i18n(
                                ...
                                visible=False)
                            lightrag_chat_checkbox = gr.Checkbox(
                                label=i18n("启用 Lightrag Chat"),  # 显示在界面上的标签
                                value=False,                       # 默认未选中
                                elem_classes="switch-checkbox",    # CSS 类，保持与其他复选框一致
                                elem_id="gr-lightrag_chat-cb",     # 元素 ID，用于前端 JavaScript 选择器
                                visible=False                      # 是否在界面上可见，根据需要设置
                            )
    ...
    with gr.Row(elem_id="popup-wrapper"):
        with gr.Box(elem_id="chuanhu-popup"):
            ...
            with gr.Box(elem_id="fake-gradio-components", visible=False):
                ...
                changeLightRAGBtn = gr.Button(
                    visible=False, elem_classes="invisible-btn", elem_id="change-lightrag_chat-btn")
    ...
    status_display = gr.Markdown(value="Processing...") 
    ...
    def submit_task(task_function, *args):
        task_queue.put((task_function, args))

    async def run_task(task_function, args):
        # 任务执行
        async for result in task_function(*args):
            # 处理每次迭代的结果
            print(result)

    async def execute_tasks():
        while True:
            task_function, args = task_queue.get()  # 获取队列中的任务
            if task_function:
                await run_task(task_function, args)  # 执行任务
    ...
    # 启动任务执行的后台线程
    executor_thread = Thread(target=start_task_executor, daemon=True)
    executor_thread.start()

    user_input.submit(
        **transfer_input_args).then(
        **LightRAG_predict_args).then(
    ...
    emptyBtn.click(
        ...
        outputs=[chatbot, status_display, historySelectList, systemPromptTxt, single_turn_checkbox, temperature_slider,
                 ...
                 presence_penalty_slider, frequency_penalty_slider, logit_bias_txt, user_identifier_txt],
        show_progress=True,
        _js='(a,b)=>{return clearChatbot(a,b);}').then(
        lambda: set_page_id(user_name, None),  # 更新该用户的 page_id 为 None 或其他值    
    )
    ...
    changeLightRAGBtn.click(
        fn=lambda value: gr.Checkbox.update(value=value),
        inputs=[lightrag_chat_checkbox],
        outputs=[lightrag_chat_checkbox],
        _js='(a)=>{return bgChangeLightragChat(a);}'
    )
